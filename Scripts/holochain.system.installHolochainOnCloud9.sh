...
if [ -f ~/workspace/Scripts/holochain.system.holochainHasBeenInstalled ] ; then
    ...
    [ -f ~/workspace/context/reality/programming/golang/go1.9.2.linux-amd64.tar.gz ] || wget https://storage.googleapis.com/golang/go1.9.2.linux-amd64.tar.gz
...
if grep -q "holochain.314" ~/.profile ; then
    ...
    if [ $whichGo = "/home/ubuntu/workspace/local/go/bin/go" ]; then
        ...
        echo "source ~/workspace/Scripts/holochain.system.configureLocalGo.sh"
        exit 1
    fi 
else
